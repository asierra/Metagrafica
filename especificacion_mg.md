...
- **Terminar el parseo** — `exit` detiene el procesamiento del archivo en ese punto; el resto se ignora. **Implementado** (2026-07-22). Es herramienta de autor: construyendo una figura por partes, deja compilar hasta donde llevas **sin comentar el resto ni mantener copias**. Va al **nivel superior del archivo**: es un control de parse-time, así que dentro de un `if` no sería condicional (la condición se evalúa después) y dentro de un bloque dejaría las llaves abiertas — anidado es error, con mensaje que lo explica. En un archivo **incluido** corta solo ese archivo y el principal sigue. Lo que sigue a `exit` no se convierte en sentencias, así que un error de **sintaxis** posterior tampoco se reporta; uno **léxico** sí (el lexer tokeniza el archivo entero antes). **No** sirve como condición de paro de una recursión (para eso, `if`, §8.1, con `max_depth` de red). *(V1: `EXIT`.)*
