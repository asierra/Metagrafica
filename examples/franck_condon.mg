...
plot(x=(0.66, rfin), y=(-0.4, ytop), box=(1.5, 1.0, 8.6, 10.4)) {
    ...
    for pozo = 0 to 1 {
        ...
        % Solo los niveles LIGADOS: pasado vmax la parábola E(v) baja, y sin la cota
        % el lazo dibujaría niveles que no existen (la misma vmax de los no resueltos).
        vmaxp = 1/(2*xe) - 0.5
        for v = 0 to 6 {
            if v < vmaxp {
                ...
                % E+elab pasa de D cerca de la disociación: ese retorno ficticio ya no
                % existe, y el rótulo se apoya en la cota lejana que fija el if de abajo.
                xl = rp+gR+0.20
