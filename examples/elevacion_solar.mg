...
R     = 5           % radio del globo
sun_x = 14.5        % los tres soles se alinean en esta columna
sun_r = 1
...
for k = 0 to 2 {
    ...
    polyline(color="darkorange", line_width=0.6) {
        (Px) (Py)  (sun_x - sun_r*cos(d*d2r)) (sy - sun_r*sin(d*d2r)) }

    circle(sun_r, fill="gold", color="darkorange") { (sun_x) (sy) }
    text("Sol", align="center", valign="middle", size=9) { (sun_x) (sy) }

    text("/b" + epoca[k], size=10) { (sun_x + sun_r + 0.5) (sy + 0.28) }
    text("$h$ = " + str(h, 1) + "°", size=8.5, color="gray") { (sun_x + sun_r + 0.5) (sy - 0.62) }
