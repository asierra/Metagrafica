...
bash test/run.sh check    # golden + gs + paridad + docs/img + negativas + galería + traducción + bloques de doc + citas + humo: ok=96 … humofail=0
...
`$(LIBDIR)` = `share/metagrafica/lib` (la ruta horneada en `-DMG_LIBDIR`), los 32 ejemplos en
...
de `examples/` (32 `.mg` × EPS/SVG/**PDF** = 96 goldens) y compara contra la red golden
(salida del propio renderer V3, regresión — no el oráculo V1). Tras tocar el motor:
`make` y `bash test/run.sh check` (debe dar **ok=96 fail=0 error=0 psfail=0 c3fail=0 imgfail=0 errfail=0 galfail=0 trfail=0 docfail=0 citafail=0 humofail=0**);
...
de versión mueve los 32 goldens EPS** (SVG y PDF no lo emiten y no se mueven); es el precio de
...
**Encabezado de un ejemplo — convención (2026-07-23).** Los 32 `.mg` del golden de `examples/` abren
...
(fuente: `main`, raíz) y enlazadas entre sí; cada README apunta a la de su idioma. 27 tarjetas
...
El parser V3 (`src/parserv3.cpp`) compila los 32 ejemplos del golden de `examples/` a
