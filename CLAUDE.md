...
bash test/run.sh check    # golden + gs + paridad + docs/img + negativas + galería + traducción + bloques de doc + citas + humo: ok=99 … humofail=0
...
`$(LIBDIR)` = `share/metagrafica/lib` (la ruta horneada en `-DMG_LIBDIR`), los 33 ejemplos en
...
de `examples/` (33 `.mg` × EPS/SVG/**PDF** = 99 goldens) y compara contra la red golden
(salida del propio renderer V3, regresión — no el oráculo V1). Tras tocar el motor:
`make` y `bash test/run.sh check` (debe dar **ok=99 fail=0 error=0 psfail=0 c3fail=0 imgfail=0 errfail=0 galfail=0 trfail=0 docfail=0 citafail=0 humofail=0**);
...
- **La GALERÍA al día** (`galfail`, nueva 2026-07-23) — `docs/galeria.html` es salida
  ...
  `docs/img`. **`galfail` cuenta además, desde el 2026-08-04, el Modelfile del agente**
  (`tools/generar_modelfile.py --check`): misma clase de fallo —salida generada que se
  pudre— y por eso comparte contador en vez de estrenar uno. Ver el Layout.
...
de versión mueve los 33 goldens EPS** (SVG y PDF no lo emiten y no se mueven); es el precio de
...
**Los tres mapas de `lib/` se generan con `--simplify 0.001`** (2026-08-31): 12582 → 6260 vértices, y la salida de sus tres figuras baja ~30 % (el SVG de `elevacion_solar`, de 128 a 86 KB). El criterio de aceptación fue **el peor píxel**, no cuántos cambian: un píxel que voltea de tierra a océano vale 93/255 en gris, y a 0.001 el peor de toda la figura se queda en 27/255 — o sea que **ningún contorno se mueve**, todo el cambio es sombreado de borde. A 0.004 el peor píxel llega a 81 % de un volteo y se aplana Baja California: **0.004 es para un LOGO de 2 cm**, y esa variante se saca con `simplifica_mg.py` donde se use y **no se committea** —un mapa decimado sirve al tamaño para el que se decimó—. ⚠️ **`--simplify 0` no es un no-op**: quita colineales exactos y duplicados consecutivos (13 traía `fulldisk_map`, o sea segmentos degenerados), un saneo sin pérdida, y por eso corre siempre. Y un anillo que se decime a menos de 3 vértices **se descarta con aviso**: sería un relleno de área cero, justo lo que caza la invariante (b) de la Capa 3. **La geometría de la llave extensible vive en `include/brace.h`** (2026-08-06), con el mismo papel y por la misma razón que `markers.h`: una fuente de verdad que los tres backends consumen. ⚠️ La llave se ARMA EN DISPOSITIVO —el vano va en mundo pero la profundidad en pt—, y por eso `Display::brace` es una virtual y no se compone con `moveto`/`lineto`/`curveto`, que transformarían sus argumentos por el marco y estirarían los ganchos con la ventana. Medido: bajo `scale 4 1` la llave sale idéntica en vez de estirarse 4×. ⚠️ **`citas.py` no es Python de conveniencia: es el único que lee el ÁRBOL para juzgar la documentación**, así que si algún día se mueve un `.mg` de `examples/` o `lib/`, lo dirá. Son auxiliares **fuera del compilador** — no se ligan a `bin/mg` ni el lenguaje depende de ellas; la política de "sin preprocesadores externos" del Code style se refiere a la compilación de un `.mg`, no a preparar datos antes. ⚠️ **`geo2mg.py` es OPCIONAL y el más pesado**: requiere `geopandas`/`pyproj`/`shapely` (los otros usan solo `pandas`) y datos Natural Earth 1:110m que **NO van en el repo** (se bajan de naturalearthdata.com; el header del tool y de cada `.mg` generado dicen cómo). Los mapas de `lib/` son assets GENERADOS committeados (como `docs/img`), con su comando de regeneración en el encabezado. `test/run_translator.sh` apunta a `tools/mg1to2.py`.
...
**Encabezado de un ejemplo — convención (2026-07-23).** Los 33 `.mg` del golden de `examples/` abren
...
(fuente: `main`, raíz) y enlazadas entre sí; cada README apunta a la de su idioma. 32 tarjetas
...
- **`Display`** (`include/Display.h`) — abstract backend + device-independent state machine. Implementations: `EPSDisplay`, `SVGDisplay`, `PDFDisplay`. ⚠️ **`dspstate.fontFace` es la cara AMBIENTE (estado lógico, la que fija `font` y salvan `gsave`/`grestore`), no la cara que el dispositivo tiene puesta** — eso son `dev_face`/`dev_size` en EPS y PDF. **Nada que no sea fijar la cara debe escribirla, y en particular CAMBIAR EL TAMAÑO NO** (2026-08-30): `EPSDisplay::setFontSize`, `PDFDisplay::setFontSize` y `Display::setRelFontSize` la ponían en `FN_NOFACE` «para forzar el setfont», o sea que usaban el estado lógico como bandera de dispositivo sucio — y así un `font_size` (o un `text(size=…)`) **borraba la cara del documento**: `font "sanserif"` seguido de un cambio de tamaño sacaba en Times-Roman todo lo que viniera después. Es la MISMA confusión que el bug de la cara ambiente, en su tercera instancia, con la misma firma: **EPS y PDF mal, SVG bien** (no tiene `dev_face`), los tres byte-estables, y el corpus ciego porque sus cuatro sentencias `font` piden `"roman"`, que ya es la cara por defecto. Forzar no hace falta: el guard de `EPSDisplay::setFontFace` compara contra `dev_face` Y `dev_size` (con `relfontsize` dentro) y PDF/SVG fijan el tamaño en cada dibujo. La cara que selecciona un trozo de texto es **local al trozo** y `Text::draw` la devuelve con `restoreAmbientFace` al terminar; **no lo quites** (2026-08-19): mientras faltó, un run `$…$` dejaba LM Math puesta para el resto del documento y bastaba un `text("$x$")` antes de un `plot` para teñir el eje entero — con `quickstart` (leyenda en itálica) y `fig6-4` (la marca «1» del eje log) saliendo así **publicados**. Y hacía discrepar a los backends: SVG salía bien **por accidente** (no tiene `dev_face`), EPS y PDF mal, sin que la Capa 3 pudiera verlo porque su invariante (a) cuenta `<tspan>`, **no compara caras**. La red es `examples/texto.mg` (sus tres últimas líneas) — cerrarlo no movió un golden, así que perderlo tampoco lo movería. Ver `docs/bitacora.md` 2026-08-19.
...
El parser V3 (`src/parserv3.cpp`) compila los 33 ejemplos del golden de `examples/` a
