...
bash test/run.sh check    # golden + gs + paridad + docs/img + negativas + galería + traducción + bloques de doc: ok=81 … docfail=0
...
`$(LIBDIR)` = `share/metagrafica/lib` (la ruta horneada en `-DMG_LIBDIR`), los 27 ejemplos en
...
de `examples/` (27 `.mg` × EPS/SVG/**PDF** = 81 goldens) y compara contra la red golden
(salida del propio renderer V3, regresión — no el oráculo V1). Tras tocar el motor:
`make` y `bash test/run.sh check` (debe dar **ok=81 fail=0 error=0 psfail=0 c3fail=0 imgfail=0 errfail=0 galfail=0 trfail=0 docfail=0**);
...
El parser V3 (`src/parserv3.cpp`) compila los 27 ejemplos del golden de `examples/` a
