...
bash test/run.sh check    # golden + gs + paridad + docs/img + negativas + galería + traducción + bloques de doc: ok=84 … docfail=0
...
`$(LIBDIR)` = `share/metagrafica/lib` (la ruta horneada en `-DMG_LIBDIR`), los 28 ejemplos en
...
de `examples/` (28 `.mg` × EPS/SVG/**PDF** = 84 goldens) y compara contra la red golden
(salida del propio renderer V3, regresión — no el oráculo V1). Tras tocar el motor:
`make` y `bash test/run.sh check` (debe dar **ok=84 fail=0 error=0 psfail=0 c3fail=0 imgfail=0 errfail=0 galfail=0 trfail=0 docfail=0**);
...
(fuente: `main`, raíz) y enlazadas entre sí; cada README apunta a la de su idioma. 25 tarjetas
...
El parser V3 (`src/parserv3.cpp`) compila los 28 ejemplos del golden de `examples/` a
